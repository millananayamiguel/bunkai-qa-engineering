...
- **Dropped with the split**: `--method=pat` (validate an existing PAT via `GET /me`, no POST) and the signup-fallback on 401 (`POST /auth/signup` then retry signin). The synced core only performs one POST and exposes no pre-auth hook, so neither flow is expressible in the adapter. Both were already dead here: no `{ENV}_{ROLE}_API_TOKEN` key exists in `.env`, and the staging owner exists so signup never fired. Re-provision a user by hand (or restore the flow from git history at `8497851:scripts/api-login.ts`) if one is ever needed again.
- **Known gap — role credentials**: `--role <r>` re-resolves `{ENV}_{ROLE}_EMAIL` / `_PASSWORD` in the adapter when `.env` provides them, and otherwise **falls back to the default user** while still naming the token `API_TOKEN_<ROLE>_<ENV>`. The synced arbiter test asserts that contract (a named role must succeed on the default credentials), so the adapter cannot fail loudly there. Only `user` is provisioned today: provision the role in `.env` before trusting a role-scoped token.
